...
public:
    vector<int> survivedRobotsHealths(vector<int>& positions, vector<int>& healths, string directions) {
        ...
        // Sort by actual position on the line
        sort(robots.begin(), robots.end(), [](const Robot& a, const Robot& b) {
            return a.pos < b.pos;
        });

        stack<int> st; // stores index in the 'robots' vector
        for (int i = 0; i < n; i++) {
            if (robots[i].dir == 'R') {
                st.push(i);
                continue;
            ...
            // Robot is moving Left, check for collisions with Right-movers
        ...
        // Sort back to original input order to extract healths
        sort(robots.begin(), robots.end(), [](const Robot& a, const Robot& b) {
            return a.id < b.id;
        });
